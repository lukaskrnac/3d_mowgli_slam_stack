# Kópia upstream launch/lidar_localization.launch.py z rsasaki0109/lidar_localization_ros2
# (commit 537ac5b, 2026-09-19), BSD-2-Clause.
#
# Jediná zmena oproti upstreamu: argument `tf_topic` a remap ('/tf', tf_topic).
# Predvolene ide TF výstup lokalizácie na /lidar_localization/tf, kde ho nikto
# nečíta, takže `map -> odom` v MowgliNext publikuje iba fusion_graph.
# Lokalizácia ide von len cez /pcl_pose a /alignment_status.
#
# Remap sa týka aj vnútorného TransformListenera, ten teda počúva
# /lidar_localization/tf + /tf_static. S enable_map_odom_tf:=false (Mode A)
# node potrebuje iba statický base_link -> lidar_frame_id (a base_link -> imu,
# ak by bol zapnutý use_imu). Mode B (enable_map_odom_tf:=true) s týmto remapom
# NEFUNGUJE, lebo nevidí odom -> base_link z /tf.
#
# Pôvodné správanie: tf_topic:=/tf
...
def generate_launch_description():
    ...
    tf_topic = LaunchConfiguration('tf_topic', default='/lidar_localization/tf')
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            'tf_topic',
            default_value='/lidar_localization/tf',
            description='Kam ide TF výstup lokalizácie. Predvolene slepý topic, aby '
                        'nekolidoval s fusion_graph v MowgliNext; /tf = upstream správanie.'),
